...
vlan_id_map = {"l2_switch":1, "int":2} # map the vlans of parrallel switches

port_slicing = {}                                       # map the slicing of ports of SUME nf[0, 1, 2, 3] based in network topology
port_slicing[0] = "int"
port_slicing[1] = "l2_switch"
port_slicing[2] = "l2_switch"
port_slicing[3] = "l2_switch"
...
def applyPkt(pkt, ingress, time, extra_len=None):
...
def expPkt(pkt, egress, drop=False):
    pktsExpected.append(pkt)
    sss_sdnet_tuples.sume_tuple_expect['dst_port'] = nf_port_map[egress]
    if egress not in ["nf1"]:
...
def make_INT_pkt(ingress, egress, icnt, max_hop, total_hop, instr_mask, data, size, vlan_id):
    pkt = Ether(dst= MAC_addr_H[dst_host_map[src_host]], src=MAC_addr_H[src_host]) / Dot1Q(vlan=vlan_id) / \
...
for time in range(DEF_PKT_NUM):
    ...
    if ( vlan_id == vlan_id_map["l2_switch"] ):
        ...
        # pkt_exp = pkt_app = Ether(src=src_MAC, dst=dst_MAC) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7) / ((DEF_PKT_SIZE - HEADER_SIZE)*"A")
        pkt_exp = pkt_app = Ether(src=src_MAC, dst=dst_MAC) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7)

    elif ( vlan_id == vlan_id_map["int"] ):
        ingress = inv_nf_id_map[src_host]
        egress = inv_nf_id_map[dst_host_map[src_host]]
        pkt_in = make_INT_pkt(ingress, egress, ins_cnt, 10, 0, SWITCH_ID_MASK, [], DEF_PKT_SIZE, vlan_id)

    else:
        print("\nERROR: vlan_id not mapped!\n")
        exit(1)

    if (inv_nf_id_map[src_host] == "nf0"):
        extra_len = ins_cnt*DATA_SIZE
        applyPkt(pkt_in, ingress, time, extra_len)
    else:
        pkt_app = pad_pkt(pkt_app, DEF_PKT_SIZE)
        ingress = inv_nf_id_map[src_host]
        applyPkt(pkt_app, ingress, time)

    if (inv_nf_id_map[dst_host_map[src_host]] == "nf1"):
        extra_len = ins_cnt*DATA_SIZE
        pkt_out = make_INT_pkt(ingress, egress, ins_cnt, 10, 1, SWITCH_ID_MASK, [src_host], len(pkt_in)+extra_len, vlan_id)
