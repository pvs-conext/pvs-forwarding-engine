...
class SimpleTester(cmd.Cmd):
    ...
    def _make_packet(self, flow_size, src_host):
        ...
        if ( vswitch == "l2_switch" or vswitch == "l2_switch1" or vswitch == "l2_switch2" ):
            ...
            pkt = Ether(src=src_MAC, dst=dst_MAC)  / IP(src=src_IP, dst=dst_IP) / UDP(sport=sport, dport=dport) / ((flow_size - HEADER_SIZE)*"A")
            ...
            # pkt = Ether(src=src_MAC, dst=dst_MAC) / Dot1Q(vlan=vlan_id, prio=vlan_prio) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7) / UDP(sport=sport, dport=dport) / ((flow_size - HEADER_SIZE)*"A")
            pkt = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, ttl=64, chksum=0x7ce7) / UDP(sport=sport, dport=dport) / ((flow_size - HEADER_SIZE)*"A")
            print "\n  H" +str(src_host)+ " -> H" +str(dst_host_map[src_host])
            ...
            pkt = Ether(src=src_MAC, dst=dst_MAC) / IP(src=src_IP, dst=dst_IP, ttl=20) / UDP(sport=f_sport, dport=f_dport) / ((flow_size - HEADER_SIZE)*"A")
        else:
